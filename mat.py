...
def equal(A, B):
    ...
    """
    assert A.D == B.D
    return {k:v for k,v in A.f.items() if v != 0} == {k:v for k,v in B.f.items() if v != 0}
...
def add(A, B):
    ...
    """
    assert A.D == B.D
    return Mat(A.D, {(k,v):A[k,v]+B[k,v] for k in A.D[0] for v in A.D[1]})
...
def transpose(M):
    ...
    True
    """
    return Mat((M.D[1], M.D[0]), {(y,x):z for (x,y),z in M.f.items()})
