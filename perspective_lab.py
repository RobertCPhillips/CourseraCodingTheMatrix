...
def move2board(y): 
    ...
    '''
    y3 = y['y3']
    return Vec({'y1','y2','y3'}, {'y1':y['y1']/y3,'y2':y['y2']/y3,'y3':1})
...
def make_nine_equations(corners):
    ...
    ''' 
    [u0,u1] = make_equations(corners[0][0],corners[0][1],0,0)
    [u2,u3] = make_equations(corners[1][0],corners[1][1],0,1)
    [u4,u5] = make_equations(corners[2][0],corners[2][1],1,0)
    [u6,u7] = make_equations(corners[3][0],corners[3][1],1,1)
    
    return [u0,u1,u2,u3,u4,u5,u6,u7,w]
...
H = Mat(({r for (r,_) in hvec.D},{c for (_,c) in hvec.D}),hvec.f)
...
def mat_move2board(Y):
    ...
    <BLANKLINE>
    '''
    return Mat(Y.D,{k:(v/Y['y3',k[1]]) for k,v in Y.f.items()})
